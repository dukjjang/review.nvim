...
항목은 **PENDING → REJECT → OK** 순으로 정렬하며, 같은 상태에서는 파일명·줄 순서를 따릅니다. 재진입 시 더 높은 우선순위의 항목이 있으면 그 항목부터 보여주고, 마지막 위치가 현재 최우선 상태에 속하면 해당 위치를 재개합니다. 위치 재확인이 필요한 항목은 자동 이동에서 제외합니다.
